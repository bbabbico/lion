...
(() => {
  ...
  function executeFn(fn) {
    ...
    // Show BOOM animation
    showBoomAnimation(fn.name, () => {
      // Actually execute the function
      actuallyExecute(fn.name);
      // Remove overlay
      removeOverlay();
    });
  }
  ...
  function actuallyExecute(name) {
    console.log(`[Event Blaster] Attempting to execute: ${name}`);

    // Priority 1: window[name]()
    if (typeof window[name] === 'function') {
      try {
        window[name]();
        console.log(`[Event Blaster] ✅ Executed via window['${name}']()`);
        return;
      } catch (e) {
        console.warn(`[Event Blaster] window['${name}']() failed:`, e);
      }
    }

    // Priority 2: querySelector data-action click
    const actionEl = document.querySelector(`[data-action="${name}"]`);
    if (actionEl) {
      try {
        actionEl.click();
        console.log(`[Event Blaster] ✅ Executed via data-action="${name}" click`);
        return;
      } catch (e) {
        console.warn(`[Event Blaster] data-action click failed:`, e);
      }
    }

    // Priority 3: dispatchEvent
    try {
      document.dispatchEvent(new Event(name));
      console.log(`[Event Blaster] ✅ Dispatched event: ${name}`);
      return;
    } catch (e) {
      console.warn(`[Event Blaster] dispatchEvent failed:`, e);
    }

    // Priority 4: eval fallback
    try {
      eval(name + '()');
      console.log(`[Event Blaster] ✅ Executed via eval: ${name}()`);
    } catch (e) {
      console.warn(`[Event Blaster] eval fallback failed:`, e);
    }
  }
